<?php
if(!class_exists("ValidatorMONBase")) {
	class ValidatorMONBase {
    	public $key = "92D418EE17C95288";
    	private $product_id = "22";
    	private $product_base = "validatormon";
    	private $server_host = "https://gitcoded.co.uk/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_validatormon_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=validatormon_fupc' ) . "'>Check For Updates</a>";
							}
							return $links;
						}, 10, 2 );
